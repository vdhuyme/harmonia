...
impl From<DomainError> for AppError {
    fn from(err: DomainError) -> Self {
        match err {
            ...
            DomainError::ValidationFailed(msg) => AppError::ValidationError(msg),
            ...
            DomainError::UnknownProvider => AppError::BadRequest("Unknown provider".to_string()),
            DomainError::InvalidStatus => AppError::BadRequest("Invalid status".to_string()),
            DomainError::DuplicateVote => AppError::ValidationError("Duplicate vote".to_string()),
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status, error, details) = match self {
            ...
            AppError::ValidationError(msg) => (StatusCode::BAD_REQUEST, "Validation error", Some(msg)),
            AppError::InternalError(msg) => {
                tracing::error!("Internal error: {}", msg);
                (StatusCode::INTERNAL_SERVER_ERROR, "Internal server error", None)
            }
            AppError::MissingAuthToken => (StatusCode::UNAUTHORIZED, "Missing authentication token", None),
            AppError::InvalidAuthToken => (StatusCode::UNAUTHORIZED, "Invalid authentication token format", None),
            AppError::InvalidToken => (StatusCode::UNAUTHORIZED, "Invalid or malformed token", None),
            AppError::TokenExpired => (StatusCode::UNAUTHORIZED, "Token has expired", None),
            AppError::TokenCreationFailed => {
                tracing::error!("Failed to create token");
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to create authentication token", None)
            }
