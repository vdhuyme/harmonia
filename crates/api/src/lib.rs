...
pub fn router(state: AppState) -> Router< (AppState, Arc<jwt::JwtHandler>) > {
    ...
    Router::new()
        ...
        .route("/auth/spotify/callback", post(handlers::oauth::spotify_callback))
        // Queue endpoints (protected)
        .route(
            "/songs/request",
            post(handlers::queue::request_song),
        )
        .route(
            "/queue/:room_id",
            get(handlers::queue::get_queue),
        )
        .route(
            "/queue/:room_id/vote",
            post(handlers::queue::vote_song),
        )
